...
window.onload = function(){
  if (typeof(jQuery) == 'undefined'){
    alert('ERROR: jQuery not found!\n\n' +
      'The Author Editor requires jQuery, which does not appear to be ' +
...
function onAjaxSuccess(json, onSuccess){
  ...
  if (resCode == 100){
    ...
    if ($.inArray(resCode, [108, 109]) == -1)
      $('.headline').text('Author Editor: Record #' + recID);
...
function initStateFromHash(){
  ...
  if (gState != gPrevState || (gState == 'edit' &&
    ...
    if (isNaN(recID)){
      ...
      $('.headline').text('Author Editor: Record #' + tmpRecID);
      displayMessage(102);
    ...
    cleanUp(true, '', null, null, true);
        $('.headline').text('Author Editor: Create new record');
...
function onGetRecordSuccess(json){
  ...
  gRecID = json['recID'];
  $('.headline').html(
    'Author Editor: Record #<span id="spnRecID">' + gRecID + '</span>' +
  ...
  $('.bibEditCellContentProtected').css('font-weight', 'normal')
  $('.bibEditCellContent').css('font-weight', 'bold')
  $('#content_100_0_0').trigger('click');
}
...
function onSubmitClick(){
  ...
  if (displayAlert('confirmSubmit')){
    createReq({recID: gRecID, requestType: 'submit', force: onSubmitClick.force}, 
              function(json) {
...
function bindNewRecordHandlers(){
  ...
  for (var i=0, n=gRECORD_TEMPLATES.length; i<n; i++)
    $('#lnkNewTemplateRecord_' + i).bind('click', function(event) {
      ...
      createReq({ requestType: 'newRecord', 
                  newType: 'template',
                  templateFilename: gRECORD_TEMPLATES[templateNo][0]}, 
                function(json) {
